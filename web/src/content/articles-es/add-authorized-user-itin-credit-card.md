---
title: "Cómo agregar un usuario autorizado a tu tarjeta de crédito con ITIN"
description: "Sí, los titulares de tarjetas con ITIN pueden agregar usuarios autorizados. Descubre qué emisores lo permiten, qué información necesitas y cómo afecta el crédito de ambos en 2026."
tier: detail
targetQuery: "puedo agregar un usuario autorizado a mi tarjeta de crédito si tengo ITIN"
relatedQueries:
  - "cómo agregar usuario autorizado a tarjeta de crédito con ITIN"
  - "agregar usuario autorizado sin SSN tarjeta de crédito ITIN"
  - "agregar usuario autorizado ayuda a construir crédito con ITIN"
  - "qué bancos permiten agregar usuarios autorizados con ITIN"
  - "requisitos para usuario autorizado con tarjeta ITIN 2026"
quickAnswer: "Sí, los titulares de tarjetas con ITIN pueden agregar usuarios autorizados a sus cuentas. La mayoría de los grandes emisores, incluyendo Chase y Capital One, lo permiten. Por lo general, el usuario autorizado solo necesita proporcionar su nombre y fecha de nacimiento, aunque algunos emisores también piden su SSN o ITIN."
...
faqs:
  ...
  - q: "¿El usuario autorizado que agrego necesita SSN o ITIN?"
    a: "Depende del emisor. Chase y Capital One generalmente solo requieren el nombre y la fecha de nacimiento del usuario autorizado, lo que hace posible agregar a alguien sin SSN ni ITIN. American Express y Discover suelen requerir un SSN o ITIN para verificar la identidad."
  - q: "¿Agregar un usuario autorizado afecta mi puntaje de crédito si tengo ITIN?"
    a: "Agregar un usuario no afecta directamente tu puntaje de crédito. Sin embargo, si el usuario autorizado gasta mucho y eleva el índice de utilización de la tarjeta, esa mayor utilización puede reducir tu puntaje de manera indirecta. Mantén el gasto bien por debajo del límite de crédito para proteger tu historial."
  - q: "¿La actividad del usuario autorizado aparecerá en mi reporte de crédito?"
    a: "No, pero tu historial de cuenta sí aparecerá en el reporte de crédito del usuario autorizado, que es precisamente el objetivo de este acuerdo. Tu comportamiento de pago, límite de crédito y saldo se reportan a los burós bajo el perfil del usuario autorizado."
  - q: "¿Puedo agregar un usuario autorizado que vive fuera de los Estados Unidos?"
    a: "Algunos emisores lo permiten, pero la tarjeta se enviará a una dirección en EE. UU. Chase y Capital One no requieren que el usuario autorizado tenga una dirección en EE. UU. al momento de agregarlo, pero la tarjeta física se envía a la dirección del titular principal registrada en la cuenta. Consulta con tu emisor específico antes de proceder."
published: true
---
...
Si el usuario autorizado también tiene un ITIN y quiere entender su parte en esta relación, incluyendo cómo el historial de la cuenta llega a su expediente crediticio, nuestra guía sobre [cómo convertirse en usuario autorizado con un ITIN](/es/articles/authorized-user-credit-card-itin) cubre esa perspectiva en detalle.
...
Si estás pensando en los próximos pasos para tu propia tarjeta, incluyendo si un límite de crédito más alto haría más práctico compartir la cuenta, consulta nuestra guía sobre [cómo obtener un aumento de límite de crédito con una tarjeta ITIN](/es/articles/credit-limit-increase-itin-credit-card).

---

## Preguntas frecuentes

**¿Puede un titular de ITIN agregar un usuario autorizado a su tarjeta de crédito?**
Sí. Ser el titular principal con un ITIN no te impide agregar un usuario autorizado. Las reglas del emisor para usuarios autorizados aplican de la misma manera que para los titulares con SSN. Los requisitos varían según el banco, pero la mayoría solo necesita el nombre y la fecha de nacimiento del usuario.

**¿El usuario autorizado que agrego necesita SSN o ITIN?**
Depende del emisor. Chase y Capital One generalmente solo requieren el nombre y la fecha de nacimiento del usuario autorizado, lo que hace posible agregar a alguien sin SSN ni ITIN. American Express y Discover suelen requerir un SSN o ITIN para verificar la identidad.

**¿Agregar un usuario autorizado afecta mi puntaje de crédito si tengo ITIN?**
Agregar un usuario no afecta directamente tu puntaje de crédito. Sin embargo, si el usuario autorizado gasta mucho y eleva el índice de utilización de la tarjeta, esa mayor utilización puede reducir tu puntaje de manera indirecta. Mantén el gasto combinado bien por debajo del límite de crédito para proteger tu historial.

**¿La actividad del usuario autorizado aparecerá en mi reporte de crédito?**
No, pero tu historial de cuenta sí aparecerá en el reporte de crédito del usuario autorizado, que es precisamente el objetivo de este acuerdo. Tu comportamiento de pago, límite de crédito y saldo se reportan a los burós bajo el perfil del usuario autorizado.

**¿Puedo agregar un usuario autorizado que vive fuera de los Estados Unidos?**
Algunos emisores lo permiten, pero la tarjeta se enviará a tu dirección en EE. UU. registrada en la cuenta. Chase y Capital One no requieren que el usuario autorizado tenga una dirección en EE. UU. al momento de agregarlo. Consulta con tu emisor específico antes de proceder, ya que las políticas varían.
