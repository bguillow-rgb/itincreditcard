...
//
// itincreditcard.com differs: its BaseLayout appends the suffix ONLY when the
// result still fits in 60 (guard shipped 2026-08-17), so a frontmatter title
// can use the full 60. Ported from itincreditscore 2026-09-25 with this one
// comment changed; siteName is kept in the signature so the module stays drop-in.
export function titleBudget(siteName) { // eslint-disable-line no-unused-vars
  return TITLE_RENDERED_MAX;
}
...
export function serpErrors(meta, siteName) {
  ...
  if (title.length > tMax) {
    errs.push(`title ${title.length}/${tMax}`);
  }
